...
class Tile:
    def __init__(self, left, top, right, bottom, canvas, text=''):
        ...
        self.shape = self.canvas.create_rounded_rectangle(
            left, top, right, bottom, fill='#cdc1b5')
        self.text = self.canvas.create_text(
            left + text_offset, top + text_offset, text=text)
    ...
    def value(self, value):
        ...
        font_size = FONT_SIZES[len(text)] if len(
            text) in FONT_SIZES else FONT_SIZES['big']
        font = ('Clear Sans', font_size, 'bold')
        self.canvas.itemconfig(
            self.text, fill=foreground, text=text, font=font)
...
class Game(Tk):
    ...
    def place_scoreboard(self):
        self.scoreboard = Scoreboard(
            MARGIN, MARGIN, WIDTH - MARGIN, SCOREBOARD_HEIGHT, self.canvas)

    def place_tiles(self):
        for i in range(1):
            for j in range(4):
                left, top = j * SIZE + MARGIN, i * SIZE + MARGIN + SCOREBOARD_HEIGHT
                right, bottom = (j + 1) * SIZE, (i + 1) * \
                    SIZE + SCOREBOARD_HEIGHT
...
def actualizar_valores(value_1, value_2, value_3, value_4):
    game.update_tiles([value_1, value_2, value_3, value_4])
